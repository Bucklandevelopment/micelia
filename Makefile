...
cov: ## Cobertura gate = 89% (medido 90.74% checkout limpio, hito DoD v0.1 = 70% ALCANZADO, ver docs/COVERAGE_ROADMAP.md)
	$(PYTEST) --cov=app --cov-report=term-missing --cov-report=html --cov-fail-under=89 tests/ sdk/python/tests/
	@printf "$(GREEN)✓ Reporte HTML en $(BOLD)htmlcov/index.html$(RESET)\n"
	@printf "$(YELLOW)Nota:$(RESET) gate 89%% (medido 90.74%%). Hito DoD v0.1 = 70%% alcanzado. Ver $(BOLD)docs/COVERAGE_ROADMAP.md$(RESET)\n"
