...
cov: ## Cobertura gate = 92% (medido 93.11% checkout limpio, hito DoD v0.1 = 70% ALCANZADO, ver docs/COVERAGE_ROADMAP.md)
	$(PYTEST) --cov=app --cov-report=term-missing --cov-report=html --cov-fail-under=92 tests/ sdk/python/tests/
	@printf "$(GREEN)✓ Reporte HTML en $(BOLD)htmlcov/index.html$(RESET)\n"
	@printf "$(YELLOW)Nota:$(RESET) gate 92%% (medido 93.11%%). Hito DoD v0.1 = 70%% alcanzado. Ver $(BOLD)docs/COVERAGE_ROADMAP.md$(RESET)\n"
...
_dev-banner: ## (interno) imprime el panel de URLs y puertos del ecosistema
	...
	@printf "$(BOLD)$(CYAN)║$(RESET)    Postgres       $(YELLOW)localhost:5432$(RESET)          (micelia-postgres)              $(BOLD)$(CYAN)║$(RESET)\n"
	@printf "$(BOLD)$(CYAN)║$(RESET)    Redis          $(YELLOW)localhost:6379$(RESET)          (micelia-redis)                 $(BOLD)$(CYAN)║$(RESET)\n"
	@printf "$(BOLD)$(CYAN)║$(RESET)    Ollama         $(YELLOW)http://localhost:11434$(RESET)  (micelia-ollama)                $(BOLD)$(CYAN)║$(RESET)\n"
