...
cov: ## Cobertura gate = 80% (medido 81.09%, hito DoD v0.1 = 70% ALCANZADO, ver docs/COVERAGE_ROADMAP.md)
	$(PYTEST) --cov=app --cov-report=term-missing --cov-report=html --cov-fail-under=80 tests/ sdk/python/tests/
	@printf "$(GREEN)✓ Reporte HTML en $(BOLD)htmlcov/index.html$(RESET)\n"
	@printf "$(YELLOW)Nota:$(RESET) gate 80%% (medido 81.09%%). Hito DoD v0.1 = 70%% alcanzado. Ver $(BOLD)docs/COVERAGE_ROADMAP.md$(RESET)\n"
