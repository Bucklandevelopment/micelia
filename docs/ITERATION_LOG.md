...
**Mañana (Ciclo 86):** **(a)** El eje de más valor, anotado 2 días seguidos y aún sin hacer: **convertir C83 en guard ejecutable** — test de integración que arranque el gateway con `SYSTEM_API_KEY` propia + cliente SDK real y assertee `service.registered` en el store, con `skipif` sin postgres. Hoy esa cadena solo está verificada A MANO. **(b)** Con cybertools ya en el launcher, **repetir el arranque real con canela** (`.venv`, `make run-all`, :3690) → sería el **2º slot healthy** y de paso verifica su `/health` sin `version` (DP-8). **(c)** Auditar si **más tests comparten la no-hermeticidad** que cazó hoy `test_main_boot_codex` (cualquiera que sondee `localhost:<puerto de dominio>` sin fijar el entorno): hoy salió uno por accidente al arrancar un dominio; ahora que el launcher arranca 6, la probabilidad de rojos fantasma sube. Recordatorio honesto C66–C85: **la severidad heredada del log ha estado mal 4 días de 4** (DP-6, DP-5, DP-14 y hoy DP-15: la conclusión era buena, el motivo no) → **audita el motivo, no solo la conclusión**; **el nombre de carpeta/el rol aparente no es el contrato** (cybertools "no es un frontend" era cierto e irrelevante: el registry lo necesita vivo); **ejercer el sistema real rinde** (hoy: el rojo del verify que ninguna lectura habría dado); **desactivar > borrar**; **no fabricar fix sin lector/emisor vivo**. Local-first M1: jamás `docker-full`; parar todo lo que se arranque; `.env` intocable.
