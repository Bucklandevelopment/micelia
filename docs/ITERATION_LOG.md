...
- `podman inspect idm-redis` → **project label = `deploy`**: el `idm-redis`/`idm-postgres` que EXISTEN (Exited) son del compose de **`deploy/`** (el stack de producción-local del usuario: `idm-caddy` TLS para micelia.idmmortality.com, `idm-core`, volúmenes `deploy_postgres_data`/`deploy_redis_data`), **no del compose de micelia**.
- Pero **micelia/docker-compose.yml define los MISMOS `container_name`** (`idm-redis`, `idm-postgres`). → **colisión de nombres.** El `docker-infra` de micelia no puede levantar SU redis (con `:6380`) porque el `idm-redis` de `deploy/` ocupa el nombre y está **pineado por `idm-core` de deploy** (dependiente).
- Recrear `idm-redis` exigiría **borrar el stack de producción del usuario** (idm-core/idm-caddy dependientes). **NO lo hago** — es su despliegue live (aunque hoy Exited); es una decisión irreversible sobre su entorno, no sobre dev.

- **Hecho:** ningún commit de código — **C105 es verificación**, y el arreglo real es una **decisión de arquitectura del dueño** (abajo), no un fix unilateral (renombrar container_names tocaría el contrato de deploy, pineado por `test_deploy_contract`). Único artefacto: esta entrada.
- **Verify:** `make verify` **verde** (sin cambios desde C104: `1649 passed, 19 skipped`).
- **Lo ya verificado (la cadena NO está rota, solo el arranque conjunto lo está):** C103 probó **en vivo** que education arranca contra redis-stack:6380 (`healthy=True, version=0.1.0`); C104 probó que el compose de micelia expone `:6380` (schema + pins). El único tramo sin demostrar es el `start` **totalmente automatizado**, y lo bloquea el ENTORNO (colisión con el stack de prod), no el código.
- **Higiene:** **nada removido** — `make docker-down` falló ANTES de borrar; los 5 contenedores del usuario siguen intactos (Exited, todos `deploy`-owned). Arranqué la podman machine para inspeccionar → **devuelta a stopped** (estaba parada). `:8888/:5050/:6380` libres. Cero residuales. Stack de producción del usuario **intocado**.

**DECISIÓN PENDIENTE (para Jessicache) — NUEVA, DP-21 (importante, arquitectura infra):** el `docker-infra` de **micelia** y el stack de **producción-local `deploy/`** usan los MISMOS `container_name` (`idm-redis`, `idm-postgres`, probablemente más) → **no pueden coexistir**, y aplicar cambios al compose de micelia (como el `:6380` de C104) queda **bloqueado** mientras exista el stack de deploy. **Opciones:** (a) dar a la infra DEV de micelia **nombres/red distintos** (p.ej. `midev-redis`, o un `project name` propio) para no colisionar con prod — toca `container_name` de varios servicios y el contrato de deploy (`test_deploy_contract`), decisión tuya; (b) aceptar que **dev y prod no corren a la vez**: `podman-compose -f deploy/… down` antes de `run-ecosystem.sh start`, y viceversa — documentarlo como operativa. Hasta decidir, `run-ecosystem.sh start` no levanta la infra dev limpia si el stack de deploy está presente. **DP-20 sigue CERRADA** (el compose de micelia es correcto; el bloqueo es de convivencia con prod, no del bind). Quedan **DP-17** (biohack py3.14) y **DP-1..DP-4** (funnel).

**Mañana (Ciclo 106):** **(a)** si Jessicache elige DP-21(a), renombrar la infra dev de micelia a nombres propios (`midev-*` o project name) — desacopla dev de prod y desbloquea `run-ecosystem.sh start` limpio; con test_deploy_contract actualizado. Es la que más desbloquea. **(b)** si elige DP-21(b), añadir al `start` un preflight que **avise** si un `idm-redis` ajeno (label≠micelia) ocupa el nombre, en vez del error críptico de podman + degradado silencioso. **(c)** biohack (DP-17) o el 4º mecanismo (auto-mat-ion pub/sub) si el arranque conjunto queda aparcado. Recordatorio honesto C66–C105: **ejercer destapa lo que la lectura no da** (hoy: la colisión dev/prod de container_name, invisible hasta intentar recrear); **no tocar el stack de PRODUCCIÓN del usuario para una verificación de dev** (aunque esté Exited: es su micelia.idmmortality.com); **un fix bloqueado por convivencia con prod es decisión del dueño, no unilateral**; **auditar el `project label` antes de asumir de quién es un contenedor** (idm-redis parecía "infra de micelia", era de deploy). Local-first M1: jamás `docker-full`; parar/restaurar todo; `.env`/volúmenes/stack de prod intocables.

**Estado: IMPLEMENTADO ✅** (ciclo de verificación: hallazgo DP-21 duro, stack de prod intacto, cero residuales)
...
- **Hecho:** 2 commits.
  - `bdbd755` `feat(scripts): run-ecosystem.sh setup — instala deps de dominio`: `install_node` (corre `npm install` **siempre** — es idempotente y COMPLETA un árbol incompleto, el caso ideacursi/DP-19 que un skip-por-poblado no vería); `install_venv` (recrea el `.venv` solo si falta/huérfano, nunca borra uno sano, e instala **auto-detectando** requirements.txt / pyproject|setup.py); `do_setup` (itera SERVICES por runtime, exit 1 si algo falla) + dispatch `setup)`.
  - `582cc31` `fix(scripts): setup revierte el .venv si pip falla`: ver hallazgo abajo.
- **Corrido EN VIVO (autorizado) — resultados por dominio:**
  - **DP-19 (ideacursi) → RESUELTO.** `npm install` completó el árbol; `reflect-metadata` (declarado pero ausente, el blocker de C101) ahora presente. **Verificado en vivo:** el backend NestJS ya **NO** crashea por módulo faltante (arranca y pasa de la carga de módulos).
  - **DP-18 (auto-mat-ion) → RESUELTO.** `npm install` OK.
  - **DP-17 (biohack) → FALLA CONFIRMADA (como predije).** `pandas` no compila en python 3.14: `too few arguments to function call` (Cython/C-API 3.14). El setup lo **detecta y reporta** (`✗ pip install FALLÓ (deps sin wheel para este python?)`, exit 1). **Sigue necesitando decisión del dueño:** python 3.13 (pyenv) o bump de requirements a versiones cp314. No forzado.
  - panel/biohack-fe/codking-vis: no-op (ya sanos).
- **HALLAZGO no buscado (el setup me corrige a mí):** install_venv, al fallar pip en biohack, dejaba el `.venv` **recién creado** con solo pip/setuptools/wheel — un venv cuyo intérprete SÍ arranca → el **doctor lo reportaría "OK (python 3.14.5)", FALSO**, exactamente el mismo modo de fallo que el `node_modules` incompleto de DP-19. **Fix (`582cc31`): rollback** — en el fallo de pip, `rm -rf` el `.venv` creado → estado honesto (doctor: AUSENTE). Atómico. **Verificado en vivo:** tras el fallo de biohack + el fix, el doctor vuelve a decir `.venv AUSENTE` (correcto), no un OK engañoso.
- **Estado del ecosistema post-setup (doctor, real):** **8 de 9 servicios sanos** (antes 6): los 5 node OK (auto-mat-ion ya incluido), canela+cybertools venv OK; **solo biohack ✗** (venv ausente, DP-17). El doctor no miente: refleja el rollback.
- **Verify:** **verde** — `make verify`: `1646 passed, 19 skipped` (+1), cobertura ~95%. Tests del setup (skip-si-sano, missing-workdir, sin-manifest, idempotencia, dispatch); el install real se verifica en vivo, no en la suite.
- **Higiene:** ideacursi (nodemon) parado; biohack venv a-medias **eliminado**; `:5050/:8888/:5433` libres, sin residuales. Repos hermanos: **solo su árbol de deps tocado** (autorizado), código/`.env`/WIP intactos. Cero infra levantada.

**DECISIÓN PENDIENTE (para Jessicache):**
- **DP-17 sigue ABIERTA (ahora con causa exacta):** biohack no arranca en python 3.14 porque `pandas` (y probablemente numpy/scipy pinneados) **no compilan** contra el C-API de 3.14. **Elige:** (a) instalar **python 3.13** (pyenv) y que su `.venv` use ese intérprete — `run-ecosystem.sh setup` usa `python3` del sistema, habría que parametrizarlo; o (b) **bump de requirements** de biohack a versiones con wheel cp314 (toca su código, decisión suya). Hasta entonces biohack queda como el único dominio sin ejercer.
- **NUEVA — DP-20 (ideacursi necesita redis para arrancar live):** con las deps ya resueltas, el backend de ideacursi **no completa el boot** porque exige **redis en `:6380`** (`ECONNREFUSED 127.0.0.1:6380` — no es un módulo faltante, es infra; y `:6380` es no-estándar, redis suele ir en `:6379`). Ejercer el slot `education` en vivo requiere levantar redis (subconjunto de `docker-infra`) **en el puerto que ideacursi espera**. Es INFRA/verificación, no un blocker de contrato (el contrato ya está pineado, C101). Menor.

**Mañana (Ciclo 103):** **(a)** con ideacursi deps-OK, **ejercer el slot `education` en vivo** levantando redis (docker-infra, o solo redis en `:6380`) + el gateway → cerraría por fin la verificación live del 3er slot (DP-20). **(b)** auto-mat-ion (DP-18 resuelto) ahora es arrancable — verificar su heartbeat/registro live si aporta. **(c)** biohack: si Jessicache mueve DP-17, ejercerlo; si no, volver a cobertura núcleo. Recordatorio honesto C66–C102: **una acción autorizada se ejecuta con el mismo cuidado — solo el árbol de deps, nunca el código del hermano**; **`npm install` es idempotente y completa árboles incompletos** (por eso install_node no skipea); **un install que falla debe REVERTIR** (no dejar estado engañoso — el setup me lo enseñó al dejar un venv a medias que el doctor habría malreportado); **predecir un fallo y confirmarlo ejerciendo** (pandas/3.14); **deps ≠ infra** (ideacursi: deps OK pero redis :6380 lo bloquea → DP-20 nuevo). Local-first M1: jamás `docker-full`; parar/eliminar todo lo que se arranque; `.env` intocable.

**Estado: IMPLEMENTADO ✅**

---

## 2026-07-19 — Ciclo 101 (**intento ejercer el 3er dominio (education/ideacursi) en vivo y lo BLOQUEA un hallazgo: ideacursi no arranca — su node_modules está incompleto (falta `reflect-metadata`, declarado pero no instalado). Ciclo de verificación C83-style: la verificación rinde el blocker + confirma que el contrato es sano; convierto la lectura en guard cross-repo**): tarea (a) del plan de C100 (cerrar la verificación live de los slots que faltan; hoy solo cybertools/canela ejercidos).

**Método (como C83/C87):** el doctor de C98 decía `ideacursi node_modules OK (presente)` → intento `npm run dev:backend` con `BACKEND_PORT=5050`. **Crashea:** `Error: Cannot find module 'reflect-metadata'`. Auditoría: `reflect-metadata` está **declarado** en `backend/package.json:65` (^0.2.1) pero **no instalado** en ningún `node_modules` (ni root ni backend). El árbol tiene 381+158 paquetes pero le falta uno declarado → **node_modules presente pero INCOMPLETO**. → el backend NestJS no arranca, el slot `education` no se pudo ejercer live.

**Por qué NO arreglo el doctor para cazar esto (decisión deliberada):** evalué 2 heurísticas y ambas fallan o mienten. (1) **mtime stale** (`package.json` más nuevo que `node_modules/.package-lock.json`): FALSO NEGATIVO aquí — el hidden-lock es MÁS nuevo que el package.json y aun así falta la dep. (2) **`npm ls`**: sí lo detecta y es rápido (~0.7s) pero es **notoriamente ruidoso** (peer-deps/extraneous → exit≠0 en árboles sanos) → un doctor que **grita en falso es peor** que uno que se pierde algún caso. Y crucialmente: **el doctor NO mintió** — reportó `node_modules "presente"` (verdad), no "arrancable"; la sobre-lectura fue mía. Sin heurística cheap-y-correcta, no toco el doctor.

**Por qué el contrato de education ES sano (solo lo bloquean las deps locales), verificado por lectura:** (1) ideacursi sirve health en **`/api/health`** (`main.js: setGlobalPrefix('api')` + `health.controller.js: @Controller('health')`) — justo lo que el registry sondea; (2) su `/api/health` declara **`category:'education'`, port 5050 y `version:'0.1.0'`** — a diferencia de canela, cuyo `/health` NO trae version (**DP-8**): cuando ideacursi arranque, el slot tendrá version; (3) el **`PG_POOL` es LAZY** (`new Pool()` no conecta hasta la 1ª query) → arranca degradado sin postgres. El único blocker real es la completitud de deps.

- **Hecho:** 1 commit — `0fad698` `test(ideacursi): pin del contrato de health del slot education (cross-repo) + hallazgo DP-19`. `tests/test_ideacursi_health_contract_codex.py` (3 pins cross-repo, SKIP si ideacursi ausente) convierte la lectura-de-verificación en **guard ejecutable** (patrón C82/C86/C100): Micelia sondea education en `/api/health @ :5050` ↔ ideacursi lo sirve ahí (prefix+controller) y reporta `category='education'` + `version` + port 5050. Micelia-side lee el **DEFAULT** de config (no `Settings()` → hermeticidad C89).
- **Verify:** **verde** — `make verify`: `1641 passed, 19 skipped` (+3), cobertura ~95%.
- **Disciplina C60–C101:** **mutation-verified** — cambiar el probe-path del registry (`/api/health`→`/health`) rompe el pin; los regex cross-repo **rechazan drift plausible** (`research`/`v2`/`status`/línea-sin-version). Repo hermano ideacursi **solo leído/ejecutado** (a su backend solo se le pasaron env-vars; crasheó solo).
- **Higiene:** ideacursi (nodemon) **parado**, `:5050/:8888/:5433` libres, sin node/uvicorn residual. Cero infra levantada (no hizo falta: ni siquiera arrancó). `.env` del hermano **no leído** (usé `BACKEND_PORT=5050` explícito para no depender de su `.env`).

**DECISIÓN PENDIENTE (para Jessicache):** **NUEVA — DP-19 (menor, mecánica):** ideacursi necesita un **`npm install` limpio** (`cd ideacursi-tool && npm install`, o `npm ci`) — su `node_modules` está incompleto (`reflect-metadata` declarado pero ausente) y el backend NestJS no arranca. Mismo tipo que **DP-18** (auto-mat-ion `npm install`) y **DP-17** (biohack python): **deps de dominio sin instalar**. No lo hago yo (repo hermano, rutina read-only sobre ellos). Con DP-19 resuelta, el slot `education` se ejerce live en minutos (el contrato ya está pineado). Siguen DP-18, DP-17, DP-16, la menor de C84 y DP-1..DP-4.

**Observación de rumbo (3 dominios, 3 blockers de deps):** biohack (DP-17, venv), auto-mat-ion (DP-18, npm), ideacursi (DP-19, npm) — **los 3 dominios no-ejercidos comparten la misma causa: deps de dominio sin instalar/actualizar**, no fallos de contrato. Micelia (el orquestador) está sano; lo que falta es un **`setup` del ecosistema** (un `run-ecosystem.sh setup` que instale/recree las deps de cada dominio) — candidato fuerte, PERO cruza el límite de "no tocar repos hermanos" salvo que Jessicache lo autorice explícitamente. **DECISIÓN PENDIENTE**: ¿autorizas que la rutina instale deps de dominio (npm install / venv) como parte de un `setup`, o eso queda como pasos manuales tuyos?

**Mañana (Ciclo 102):** **(a)** si Jessicache resuelve CUALQUIERA de DP-17/18/19, ejercer ese dominio live (contratos ya pineados: education C101, codking C100; biohack C91). **(b)** Si autoriza el `setup` de deps de dominio, implementarlo (destraba los 3 de una). **(c)** Si no, volver a cobertura núcleo con valor real (mutation-testing sobre `prompt_executor`/`frangels`). Recordatorio honesto C66–C101: **una verificación live rinde el blocker que la lectura estática esconde** (hoy: node_modules incompleto, que el doctor por diseño no ve); **no bolt-ear una heurística frágil** (mtime falso-negativo; npm-ls grita en falso) — mejor un hallazgo honesto que un doctor que miente; **el doctor dijo "presente", no "arrancable" — la sobre-lectura fue mía**; **convertir la lectura-de-verificación en guard cross-repo** (como C100); **auditar ambos lados**; **reportar deps del hermano ≠ instalarlas**. Local-first M1: jamás `docker-full`; parar todo lo que se arranque; `.env` intocable.

**Estado: IMPLEMENTADO ✅**

---

## 2026-07-19 — Ciclo 100 🎯 (**MILESTONE. Vuelvo al núcleo de contratos: pineo el ingest PUSH-only de codking, la asimetría anotada desde DP-12/C84 y nunca convertida en guard. codking empuja eventos (source=codking) pero NO tiene slot en el registry — visible por PUSH, invisible por PULL, a propósito**): tarea (a) del plan de C99. Cierra el hueco de contrato inter-proyecto más viejo sin pin.

**La asimetría (auditada en AMBOS lados, no asumida — lección C84/C89):** codking comparte `:8000` con cybertools → Micelia **no lo sondea** por `/health` (no es uno de los 6 slots del registry). Sus eventos llegan por el **ingest** (`POST /api/v1/events`) con `source=codking`, un canal **independiente** del PULL (C92 pineó la independencia PUSH/PULL en general; C100 pinea el caso codking, que es PUSH puro). Verificado:
- **Micelia:** `source` admite cualquier string; `_normalize_source` solo reescribe el legacy `idm-core→micelia`, así que `codking` se persiste **literal**; el registry declara 6 slots (`health/research/education/security/devtools/testlab`) y codking **no está**.
- **codking (SDK hermano):** `integrations/vital_sdk/config.py` → `service_name="codking"`; `client.py::publish_event` manda `source=self.config.service_name`, hace `raise_for_status()` y devuelve `resp.json()` → depende de un **2xx + el shape** (`EventCreateResponse`), no de campos concretos.

- **Hecho:** 1 commit — `30d08a4` `test(codking): pin del contrato de ingest PUSH-only de codking (DP-12/C84, nunca pineado)`. `tests/test_codking_ingest_contract_codex.py` (5 tests):
  - **estáticos (siempre):** codking **NO es slot** del registry (`HEALTH_ENDPOINTS` == los 6, sin codking); `_normalize_source("codking")=="codking"` **literal** (contraste con `idm-core→micelia`, para no pinear en vacío); **cross-repo** SKIP-si-ausente del SDK de codking (`source=service_name` + dependencia del shape).
  - **e2e (`require_postgres`):** un evento `source=codking` (`event_type=threat.detected`) **entra por el ingest, se persiste y es consultable por source**, mientras codking sigue **SIN slot en el registry vivo** (la asimetría PUSH-sí/PULL-no, en vivo); y el ingest lo **acepta con `codking_enabled=False`** (el flag del **motor de IA** de codking, `ai.py`, NO gatea el ingest de eventos — independencia, eco de C92).
- **Verify:** **verde** — `make verify` sin postgres: `1638 passed, 19 skipped` (+3 estáticos; +2 e2e skip), cobertura ~95%.
- **Ejercido en el SISTEMA REAL:** con postgres efímero aislado (`micelia-test-pg` :5433, creds propias, sin tocar el `idm-postgres` del usuario ni `.env`) los **5 pasan**; el evento de codking se persiste y se recupera por `source=codking`.
- **Disciplina C60–C100:** **mutation-verified** — normalizar codking rompe el pin literal Y el e2e de persistencia (query por `source=codking` no lo halla); añadir codking a `HEALTH_ENDPOINTS` rompe el no-slot. No tautológicos. Repo hermano codking **solo leído**.
- **Higiene:** el e2e corre in-process (ASGITransport), sin gateway ni uvicorn. Postgres efímero **eliminado**, `idm-postgres` del usuario **devuelto a Exited** (volumen/`.env` intactos), **podman machine detenida** (estaba parada). `:8888/:5433` libres. Sin residuales.

**DECISIÓN PENDIENTE (para Jessicache):** **ninguna nueva.** El contrato de codking queda pineado en su forma correcta (PUSH-only, sin slot). Siguen **DP-18** (auto-mat-ion `npm install`), **DP-17** (biohack python), **DP-16** (venvs huérfanos, cazados por el doctor), la menor de C84 y **DP-1..DP-4**.

**Balance del centenar (C1–C100):** el ecosistema Micelia tiene hoy sus contratos núcleo **pineados y ejecutables**: registro (C86), heartbeat (C88), registry-pull (C90), cross-mechanism PUSH/PULL (C92), funnel register→login→refresh + render de errores (C93-96), ingest PUSH-only de codking (C100); su arrancabilidad local **saturada con red de seguridad** (launcher + doctor + preflight, C85/C87/C97-99); y una **disciplina de hermeticidad** barrida en 3 ejes (C85/C86/C89). Lo que queda es sobre todo **decisiones del dueño** (infra del funnel DP-1..4; deps de dominio DP-16..18) y **cobertura incremental**.

**Mañana (Ciclo 101):** con los contratos núcleo pineados, los vectores por valor: **(a)** el 6º dominio del registry aún sin ejercer live — o **ideacursi** (`/api/health`, node) o **biohack** si Jessicache mueve DP-17 — para cerrar la verificación live de los slots que faltan (hoy solo cybertools/canela ejercidos). **(b)** Cobertura donde el mutation-testing revele huecos REALES en módulos núcleo con menos pins (p.ej. `prompt_executor`, `frangels`), no tautológicos. **(c)** Si Jessicache prioriza infra, **preparar** (no decidir) DP-1..DP-4. Recordatorio honesto C66–C100: **auditar ambos lados antes de pinear un contrato cross-repo** (hoy: Micelia + el SDK de codking); **contraste anti-vacío** en los pins (`idm-core→micelia` junto al `codking` literal); **la asimetría es el contrato** (codking PUSH-only no es un bug, es diseño DP-12); **independencia de mecanismos** (ingest ⟂ flag de IA); **reportar/pinear ≠ tocar el hermano**. Local-first M1: jamás `docker-full`; parar/eliminar todo lo que se arranque; `.env` intocable.
...
- **Hecho:** 1 commit — `7e1aa75` `feat(scripts): doctor cubre también los servicios node — diagnóstico de arrancabilidad completo`.
  - **`check_node LABEL WORKDIR HINT`**: `node_modules` presente-y-poblado → **OK**; ausente o **dir vacío** (npm interrumpido) → **AUSENTE** con la pista `npm install`. No hay estado HUÉRFANO (node_modules no cuelga de un intérprete que pueda desaparecer, a diferencia de un venv — asimetría real, no descuido).
  - **`do_doctor`** ahora recorre `SERVICES` en **dos bloques agrupados** (python `.venv` / node `node_modules`), reporta la versión de **python Y de node** del sistema, y sale 1 si cualquier runtime tiene deps ausentes.
- **Corrido EN VIVO hoy — HALLAZGO no buscado:** el doctor **destapó un blocker nuevo, invisible hasta ahora**: **auto-mat-ion tiene `node_modules` AUSENTE** → no arrancaría `npm run demo`. Mismo tipo de sorpresa-al-arrancar que biohack, ahora visible de un vistazo. Snapshot completo: *python 3.14.5 / node v20.18.1 · **python:** biohack ABSENT, canela+cybertools OK · **node:** auto-mat-ion ABSENT, panel+biohack-fe+ideacursi+codking-vis OK · exit 1*. **Justo la razón de ser del doctor: convertir en diagnóstico lo que era fallo críptico.**
- **Verify:** **verde** — `make verify`: `1632 passed, 17 skipped` (+3 netos), cobertura ~95%.
- **Disciplina C60–C98:** +3 tests `check_node` (poblado→OK, ausente→AUSENTE, **dir-vacío→AUSENTE**) y los tests de agregación **reescritos** para cubrir ambos runtimes. **El pin de C97 "los servicios node NO se chequean" se INVIERTE a propósito** (igual que C85 invirtió el guard de C72): C97 lo pineaba porque entonces era cierto; C98 lo cambia y actualiza el test — no se borra a ciegas. Todo hermético (SERVICES inyectado, venvs/node_modules de mentira en `tmp_path`). **Mutation-verified:** ignorar el dir-vacío rompe la detección de vacío; saltarse el bloque node rompe la cobertura de ambos runtimes.
- **Higiene / guardarraíles:** cambio scripts+test, **read-only** (el doctor NO instala las deps de auto-mat-ion — reportar, no arreglar; instalar deps de un repo hermano queda fuera de esta rutina). Cero servicios/infra. `.env` no leído. Sin residuales.

**DECISIÓN PENDIENTE (para Jessicache):** **ninguna nueva de decisión**, pero **un hallazgo accionable nuevo**: **auto-mat-ion necesita `npm install`** (node_modules ausente) para poder arrancar como dominio `testlab`. Es del mismo tipo que DP-17 (deps sin instalar); lo anoto como **DP-18** (menor, mecánico): correr `cd auto-mat-ion && npm install` cuando quieras arrancar ese dominio — no lo hago yo porque es un repo hermano y la rutina es read-only sobre ellos. Siguen abiertas **DP-17** (biohack python 3.14), **DP-16** (venvs huérfanos, ahora con el doctor que los caza), la menor de C84 y **DP-1..DP-4**.

**Mañana (Ciclo 99):** el doctor ya da el diagnóstico de arrancabilidad completo del ecosistema (python + node). Vectores por valor: **(a)** integrar el doctor en el arranque — que `run-ecosystem.sh start` corra el doctor ANTES y avise/aborte si hay blockers, en vez de descubrirlos servicio a servicio (cierra el bucle: diagnóstico → prevención). **(b)** Si Jessicache desbloquea DP-17(b)/DP-18, completar los arranques live que faltan (biohack 3er dominio, auto-mat-ion 4º). **(c)** Volver a un contrato inter-proyecto o cobertura con valor real donde el mutation-testing revele huecos. Recordatorio honesto C66–C98: **un diagnóstico completo destapa lo que el parcial esconde** (hoy: el doctor de node encontró auto-mat-ion, que el de solo-venv no veía); **invertir un pin propio cuando la realidad cambia, actualizando el test, no borrándolo** (C98↔C97 como C85↔C72); **reportar ≠ arreglar** (el doctor no toca el repo hermano); **asimetría real ≠ descuido** (node no tiene estado huérfano). Local-first M1: jamás `docker-full`; parar todo lo que se arranque; `.env` intocable.

**Estado: IMPLEMENTADO ✅**

---

## 2026-07-19 — Ciclo 97 (**PIVOT de vuelta al núcleo tras exprimir el funnel: convierto DP-16/DP-17 de sorpresa-al-arrancar en diagnóstico temprano. Un `doctor` que reporta de un vistazo qué venvs del ecosistema están rotos, sin arrancar nada — y en vivo confirma que biohack es el único blocker**): tarea (a) del plan de C96. DP-16 (venvs huérfanos por bumps de python de brew, C87) y DP-17 (biohack sin venv arrancable en 3.14, C91) hasta hoy se descubrían **al intentar arrancar** un servicio y ver el fallo críptico. C97 los hace visibles ANTES.

- **Hecho:** 1 commit — `d566fee` `feat(scripts): run-ecosystem.sh doctor — diagnóstico temprano de venvs (DP-16/DP-17)`.
  - **`venv_python_ok WORKDIR`**: se factoriza el check de venv-huérfano de C87 a un **predicado único** que ahora comparten `start_svc` (para no arrancar un venv muerto) y el doctor (para diagnosticarlo) → sin drift entre ambos. El refactor es **behavior-preserving** (el test de `start_svc` de C87 sigue verde).
  - **`check_venv` / `do_doctor`**: diagnóstico **read-only** (no arranca ni instala) que itera la tabla `SERVICES` (fuente única del mapa) filtrando los servicios python (`marker=.venv`) y reporta por cada uno: **AUSENTE** / **HUÉRFANO** (intérprete roto) / **OK (python X.Y.Z)**; sale **1** si algo impediría arrancar.
  - **dispatch**: nuevo subcomando `doctor`; ayuda de uso actualizada.
- **Corrido EN VIVO hoy** (el valor, no solo el test): `scripts/run-ecosystem.sh doctor` → *"python del sistema: 3.14.5 · biohack API: **.venv AUSENTE** · Canela: **OK (3.14.5)** · Cybertools: **OK (3.14.5)** → exit 1"*. El blocker de DP-17 **visible de un vistazo**, con el comando de recreación al lado — sin arrancar nada, sin sorpresa.
- **Verify:** **verde** — `make verify`: `1629 passed, 17 skipped` (+7), cobertura ~95%.
- **Disciplina C60–C97:** `tests/test_ecosystem_venv_doctor_codex.py` (7 tests) sourcea el script (como C87) y ejerce `check_venv`/`do_doctor` contra venvs de mentira en `tmp_path` y un `SERVICES` **inyectado** — **hermético**, sin depender de los venvs reales de la máquina (esa dependencia sería justo la no-hermeticidad que C89 barrió; el `doctor` sobre el ecosistema real NO se asserta, solo su lógica con entradas controladas). **Mutation-verified:** tratar el intérprete como si siempre arrancara rompe la detección de huérfano; no marcar `any_bad` rompe el exit-1; no filtrar rompe el guard de no-python. Fuente restaurada.
- **Higiene:** cambio de scripts+test, read-only; cero servicios/infra arrancados; `.env` no leído. Sin residuales.

**DECISIÓN PENDIENTE (para Jessicache):** **ninguna nueva.** El `doctor` **no decide** el tooling (no instala python 3.13 ni bumpea deps) — solo diagnostica, como pide el guardarraíl. **DP-16/DP-17 siguen abiertas** pero ahora con visibilidad: para desbloquear biohack (3er dominio live) hace falta tu decisión — **DP-17(a)** bumpear los requirements de biohack a versiones con wheel `cp314`, o **DP-17(b)** instalar python 3.13 (pyenv) para su venv. La **menor de C84** (slot `devtools`) y **DP-1..DP-4** (INFRA del funnel, ya cerradas en decisión, faltan pasos de panel) siguen igual.

**Mañana (Ciclo 98):** con el funnel exprimido (C93-96) y el diagnóstico de venvs listo (C97), los vectores por valor: **(a)** si Jessicache decide **DP-17(b)** (python 3.13), completar el arranque live de biohack — el contrato ya pineado (C91), el doctor ya dirá cuándo su venv esté sano. **(b)** El eslabón de arrancabilidad que el doctor NO cubre: **verificar que los servicios NODE del ecosistema** (panel, ideacursi, automation, codking-vis, biohack-fe) tienen `node_modules` — el doctor solo mira venvs python; un `doctor` que también reporte los `node_modules` ausentes completaría el diagnóstico de arrancabilidad de TODO el ecosistema (mismo patrón, marker `node_modules`). **(c)** Cobertura con valor real donde el mutation-testing revele huecos, o volver a endurecer un contrato inter-proyecto. Recordatorio honesto C66–C97: **convertir una sorpresa-al-arrancar en diagnóstico temprano** (el doctor hace visible lo que DP-16/17 escondían); **factorizar el check duplicado a un predicado único** (venv_python_ok, sin drift); **diagnosticar ≠ decidir** (el doctor reporta, no instala); **un test de lógica de shell se hace hermético inyectando sus entradas** (SERVICES), no leyendo el estado real. Local-first M1: jamás `docker-full`; parar todo lo que se arranque; `.env` intocable.
...
- **Hecho:** 1 commit — `ec9aadc` `test(funnel): tripwire del render de errores en /register y /login (§5 "409 mostrado en UI")`. `tests/test_funnel_error_rendering_codex.py` (6 pins estáticos read-only, mismo precedente que `test_funnel_routing`): (1) ambas páginas tienen estado `error`/`setError`; (2) el `catch` del submit llama `setError(err.message)` — surfacea el mensaje del BACKEND, no un texto fijo; (3) el JSX renderiza `{error && … {error} …}` (el guard **y** la interpolación del valor); (4) `authApi.register` mapea el **409 → 'Email already registered'** (el caso literal del §5, no un genérico); (5) `authApi.login` lanza `ApiError` ante `!res.ok` (un 401 llega al catch); (6) `ApiError extends Error` + `super(message)` — el eslabón que hace que `err.message` (lo que la página renderiza) lleve el texto mapeado (si dejara de extender Error, `err instanceof Error` sería falso y la página caería al texto genérico).
- **Verify:** **verde** — `make verify`: `1622 passed, 15 skipped` (+6), cobertura ~95%.
- **Disciplina C60–C95:** los 6 pins son **mutation-verified** — quitar el `{error &&}` render, el `setError` del catch, la rama 409 del mapeo, o el `extends Error` → rompe el pin correspondiente. Ficheros frontend restaurados (git limpio; solo `tsbuildinfo` residual, pre-existente).
- **Higiene / guardarraíles:** cambio test-only, solo lectura de frontend committed. Cero servicios/infra. **NO** toqué infra/DNS/TLS/secretos ni el runbook (WIP). `.env` no leído. Sin residuales.

**HITO — preflight LOCAL del funnel SATURADO:** las 3 capas automatizables del §5 están cubiertas y ejecutables — **cableado** (C93, frontend↔backend), **flujo** (C94, register→login→acceso contra el UserStore real) y **render de errores** (C95, el 409/401 mostrado al usuario). Lo que queda del §5 es **⏳HUMANO** (flujo en el dominio real tras DNS+port-forward; rollback de infra) — no automatizable por diseño.

**DECISIÓN PENDIENTE (para Jessicache):** **ninguna nueva.** El funnel local está preflight-completo; los usuarios reales los desbloquean **solo** los pasos de panel (IONOS A-record + API key DDNS, port-forward TP-Link, `pmset`) del §2/§3 del runbook. Siguen abiertas **DP-17** (biohack en 3.14) y **DP-16** (venvs huérfanos).

**Mañana (Ciclo 96):** con el preflight local del funnel saturado, quedan vectores menores de funnel y otros abiertos: **(a)** el **refresh token** en el flujo del funnel (anotado en C94, aún sin ejercer): register/login devuelven `refresh_token` pero ni el preflight ni ningún test conduce el `/auth/refresh` de punta a punta contra el store real → un usuario con access expirado; cerrarlo completa el ciclo de sesión (e2e con postgres, como C94). **(b)** El **logout** (`authApi.logout` limpia tokens; ¿hay ruta/estado que pinear? — menor). **(c)** Si el funnel local ya no rinde valor nuevo, **volver al núcleo**: DP-17(b)/DP-16 (desbloquear biohack con python 3.13) o cobertura con valor real. **Nota de rumbo:** 3 ciclos (C93-95) han saturado lo que la rutina PUEDE hacer del funnel sin infra; a partir de aquí el funnel depende de pasos humanos de Jessicache — conviene no forzar más pins de funnel marginales y volver a donde haya valor real. Recordatorio honesto C66–C95: **una capa nueva (el render) se pinea leyendo las 3 capas que la alimentan, no asumiendo**; **saturado un vector, reconocerlo y pivotar** (no exprimir pins marginales); **preparar ≠ decidir**; **leer código committed = OK; tocar WIP/infra/`.env` = NO**. Local-first M1: jamás `docker-full`; parar todo lo que se arranque; `.env` intocable.

**Estado: IMPLEMENTADO ✅**

---

## 2026-07-19 — Ciclo 94 (**segundo paso del funnel: la cadena `register→login→acceso` completa, e2e contra el UserStore REAL sobre postgres. C93 pineó el CABLEADO (frontend↔backend); C94 pinea el FLUJO — el preflight local que 94 ciclos nunca ejercieron contra persistencia real, solo con stores falsos**): tarea (a) del plan de C93. Ataca los ítems **LOCALES** del §5 del runbook (*"register persiste (e2e local)"* + round-trip de login); los ítems `[ ]` que exigen el dominio real siguen ⏳HUMANO, fuera de scope.

**El hueco real (verificado):** el flujo del funnel se probaba solo con stores **FALSOS** — `test_auth_endpoints` usa un stub; `test_auth_funnel_integration` corre bcrypt real pero con un user_store **en memoria** (`UserModel` es Postgres-only, sin SQLite → no había forma hermética de usar el store real en `make verify`). Nadie había conducido el funnel entero contra **persistencia real**.

- **Hecho:** 1 commit — `311e59c` `test(funnel): preflight LOCAL e2e register→login→acceso contra el UserStore real`. `tests/test_funnel_preflight_smoke_codex.py` (3 tests, exigen postgres → skipif, reusan `require_postgres`+`no_domain_probes`). Se arranca el **LIFESPAN REAL** (trae el **UserStore real sobre postgres**) y se conduce el funnel por los **endpoints reales** (ASGITransport):
  - **register → login → acceso:** register **201** + **PERSISTE de verdad** (`get_user_by_email` lo encuentra en el store real); login **200** por la **ruta de USUARIO REAL** —no el fallback admin—, probado decodificando el `sub` del token == `user_id` persistido; el access_token **concede entrada autenticada** (`/me` responde con `auth_identity == user_id`). Es el preflight que C83/C87 hicieron a mano para los DOMINIOS, ahora para el **usuario final** del funnel.
  - **email duplicado → 409** (el UNIQUE del store real, §5 del runbook — antes solo contra stub).
  - **password equivocada → 401** (bcrypt real contra el hash persistido).
- **Verify:** **verde** — `make verify` sin postgres: `1616 passed, 15 skipped` (+3 que skip sin pg), cobertura ~95%.
- **Ejercido con postgres real:** contenedor efímero aislado `:5433` (propio, sin tocar el `idm-postgres` del usuario ni `.env`) → **3 passed**. **Mutation-verified:** si el login ignora el `user_store` (cae al fallback admin) el happy path **rompe** (login 401 para un email random → prueba que la aserción `sub==user_id` NO pasa vacuamente por el fallback); si register no persiste, la aserción de persistencia **rompe**. Fuente restaurada (git limpio).
- **Higiene / guardarraíles:** e2e in-process, nada de gateway/uvicorn arrancado; postgres efímero **eliminado**, **podman machine detenida**. **NO** toqué infra/DNS/TLS/secretos/`deploy/` ni el runbook (WIP). `.env` no leído. `:8888/:5433` libres. Sin residuales.

**HITO parcial del funnel:** con C93 (cableado frontend↔backend) + C94 (flujo register→login→acceso contra persistencia real), la **mitad LOCAL/automatizable del preflight §5 está cubierta y ejecutable**. Lo que resta del §5 es **⏳HUMANO** (flujo en el dominio real tras DNS+port-forward; rollback de infra) — solo Jessicache.

**DECISIÓN PENDIENTE (para Jessicache):** **ninguna nueva.** El funnel avanza: DP-1..DP-4 cerradas (runbook), preflight local cubierto (C93+C94). Lo que desbloquea usuarios reales son los pasos de panel (IONOS A-record, API key DDNS, port-forward TP-Link, `pmset`) del §2/§3/§5 del runbook — **solo tú**. Siguen abiertas **DP-17** (biohack en 3.14) y **DP-16** (venvs huérfanos).

**Mañana (Ciclo 95):** el preflight LOCAL del funnel está saturado; los vectores vivos: **(a)** el flanco que C93 dejó anotado y sigue sin pin — el **render de estados de error** en las páginas `/register`/`/login` (el §5 menciona *"409 mostrado en UI"*; hoy se pinea el contrato de red y el flujo, no que la página MUESTRE el 409/401 al usuario). Sin runner de frontend, lo pineble es que la página tenga el manejo (`catch`/estado de error que renderiza el mensaje) — un pin estático como los de C93, no un render real. **(b)** El **refresh token** en el flujo del funnel: register/login devuelven `refresh_token` pero el preflight no ejerce el `/auth/refresh` → login largo del usuario sin re-login; cerrarlo completa el ciclo de sesión. **(c)** Si Jessicache ejecuta los pasos de panel, el §5 del runbook tiene el checklist e2e-en-dominio-real. Recordatorio honesto C66–C94: **un flujo se prueba contra persistencia REAL, no contra stores falsos** (hoy: el hueco eran los fakes en memoria); **probar la ruta correcta, no una que pase por casualidad** (el `sub==user_id` descarta el fallback admin); **preparar ≠ decidir**; **leer código committed = OK; tocar WIP/infra/`.env` = NO**. Local-first M1: jamás `docker-full`; parar/eliminar todo lo que se arranque; `.env` intocable.

**Estado: IMPLEMENTADO ✅**

---

## 2026-07-19 — Ciclo 93 (**PIVOT estratégico: tras 92 ciclos endureciendo el núcleo, doy el primer paso hacia el objetivo REAL del proyecto — el funnel de captación. No tomo decisiones de infra (son de Jessicache) ni toco el runbook (WIP): endurezco el CABLEADO frontend↔backend del funnel, que estaba 100% sin guard pese a que el backend de auth está al 100% de cobertura**): tarea (a) del plan de C92, la deuda estratégica que señalé. Orientación primero (había WIP y un workstream paralelo que respetar).

**Lo que encontré al orientar (no asumí):** el funnel `register→login→micelia` está **mucho más avanzado de lo que el log daba a entender** — `docs/FUNNEL_IDMMORTALITY_RUNBOOK.md` (WIP, 139 líneas) documenta **DP-1..DP-4 CERRADAS el 2026-07-17** (hosting local M1 + Caddy TLS + port-forward TP-Link; secretos en `deploy/.env`; postgres de prod = la del compose con backup diario). Lo que queda son **pasos humanos** (registro A en IONOS, API key DDNS, port-forward del router, `pmset`) — **⛔ fuera de mi scope por diseño**. El propio §6 del runbook asigna a esta rutina exactamente: *"endurecer + validar las rutas nativas del funnel + sus tests"*. Eso es lo que hago.

**El gap real (verificado):** el funnel NATIVO sirve `/register` y `/login` como rutas de la app Next (`frontend/`), que hablan con el backend por un proxy same-origin `/api/v1/*`. El backend (`app/api/v1/auth.py`) está al **100%** y su contrato (register 201/409/503, login, duplicados) ya pineado. PERO el **cableado frontend↔backend** —`página → authApi.{register,login} → POST /api/v1/auth/... → rewrite de next.config → gateway → auth.py`— **no lo verificaba NADA**: ni `tsc`/lint (no entienden la semántica) ni los tests de backend (pegan a la ruta directa, ciegos a lo que el frontend llama).

- **Hecho:** 1 commit — `c7d04ac` `test(funnel): tripwire del cableado register→login (frontend↔backend), scaffolding C93`. `tests/test_funnel_routing_codex.py` (6 pins read-only, mismo precedente que `test_ecosystem_ports`): (1) **puerta del funnel** — `/register` y `/login` en `PUBLIC_PATHS` del middleware (sin `/register` público, el middleware redirige al nuevo usuario a `/login` → **cero captación**, y ni un rojo ni el `tsc` avisa); (2) esas rutas resuelven a páginas Next reales (`app/register|login/page.tsx`); (3) el no-autenticado se redirige a `/login`; (4) `next.config` reescribe `/api/v1/:path*` → gateway (`MICELIA_URL/IDM_CORE/:8888`) — el puente frontend→backend (aprendizaje C-2026-07-17: se hornea en build); (5) `authApi.{register,login}` POSTean al **MISMO path** que sirve el router de auth (`prefix`+ruta) — si divergen, el funnel da **404 en prod**; (6) el body (`{email,password}`/`{username,password}`) coincide con `Register/LoginRequest`.
- **Verify:** **verde** — `make verify`: `1616 passed, 12 skipped` (+6), cobertura ~95%.
- **Disciplina C60–C93:** los pins están **mutation-verified** — quitar `/register` de `PUBLIC_PATHS`, driftar el path de `authApi.register`, o romper el rewrite → rompe el pin correspondiente. Ficheros frontend restaurados (git limpio; el único diff residual es el `tsbuildinfo`, artefacto pre-existente).
- **Higiene / guardarraíles respetados a rajatabla:** **NO** toqué el runbook (WIP de Jessicache), **NO** toqué infra/DNS/TLS/secretos/`deploy/`, **NO** `git push`. Solo lectura de código frontend+backend (committed, no WIP) + un fichero de test nuevo. Cero servicios arrancados. `.env` no leído.

**DECISIÓN PENDIENTE (para Jessicache):** **ninguna nueva.** El funnel avanza por su workstream humano (DP-1..DP-4 ya cerradas en el runbook; faltan los pasos de panel IONOS/router/pmset que **solo tú** puedes dar — ver §2/§3/§5 del runbook). **Nota de coherencia:** el `ITERATION_LOG` venía listando "DP-1..DP-4 (INFRA del funnel)" como abiertas cuando el **runbook las marca cerradas desde el 2026-07-17** — la verdad es que la MITAD DECISIÓN está cerrada y lo que resta es EJECUCIÓN HUMANA de panel; a partir de C93 lo describo así. Siguen abiertas **DP-17** (biohack en 3.14) y **DP-16** (venvs huérfanos).

**Mañana (Ciclo 94):** el pivot al funnel abre vectores nuevos, todos dentro de scope (§6: endurecer rutas nativas), sin tocar infra: **(a)** **preflight LOCAL automatizable del funnel** — un smoke que arranque el gateway + valide la cadena `register→login→acceso` end-to-end contra el store (extiende el guard de auth con un flujo completo, no solo endpoints sueltos; requiere postgres → `skipif` como los demás e2e). Ataca el §5 del runbook (los ítems `[ ]` que son LOCALES, no los del dominio real). **(b)** Endurecer la página de `/register`/`/login` en sí (estados de error 409/401 renderizados — §5 menciona "409 mostrado en UI"; hoy solo se pinea el contrato de red, no el render). **(c)** Si Jessicache ejecuta los pasos de panel, el runbook §5 tiene el checklist e2e-en-dominio-real listo. Recordatorio honesto C66–C93: **orientar ANTES de actuar cuando hay WIP/workstream paralelo** (hoy: leí el runbook y descubrí que el funnel estaba más avanzado de lo que asumía — habría duplicado o chocado si scaffoldeo a ciegas); **el scope lo define el propio runbook (§6), no mi entusiasmo**; **pinear el cableado cross-capa que ni tsc ni el backend ven**; **preparar ≠ decidir** (endurezco rutas; no toco DNS/TLS/secretos); **leer código committed = OK; tocar WIP/infra/`.env` = NO**. Local-first M1: jamás `docker-full`; parar todo lo que se arranque; `.env` intocable.

**Estado: IMPLEMENTADO ✅**

---

## 2026-07-19 — Ciclo 92 (**cierro el e2e cross-mecanismo que llevaba desde C90 solo en prosa: UN gateway viendo al MISMO dominio por sus DOS canales a la vez. Lo nuevo no es que cada mecanismo funcione (ya pineado C86/C88/C90) — es demostrar que COEXISTEN y que son INDEPENDIENTES, la tesis exacta de C83**): tarea (a) del plan de C91. C83 descubrió que Micelia tiene DOS mecanismos INDEPENDIENTES para saber de un dominio — **PUSH** (el dominio hace `POST /api/v1/events`: register+heartbeat → Event Store) y **PULL** (el registry sondea el `/health` del dominio → slot healthy) — y los vio AMBOS a la vez sobre cybertools (`store={service.registered:1, service.heartbeat:2}` + `security→healthy`). C86/C88/C90 pinearon cada uno **por separado**; faltaba pinear que **coexisten** y que **no están acoplados**.

- **Hecho:** 1 commit — `263b557` `test(e2e): cross-mecanismo — un gateway ve al mismo dominio por PUSH y PULL, independientes`. `tests/test_cross_mechanism_e2e_codex.py` (2 tests, exigen postgres → skipif). **Diseño fiel + hermético:** se arranca el **LIFESPAN REAL** del gateway, que trae SUS DOS subsistemas reales — `app.state.event_store` (destino del PUSH) y `app.state.service_registry` (motor del PULL) — así que es UN gateway, no dos mitades pegadas. El dominio es el mismo por ambos lados: `source='cybertools'` (PUSH) sirviendo el slot `security` (PULL), igual que C83.
  - **Test 1 (coexistencia + independencia):** (1) **PULL solo** → el registry sondea un `/health` real en un **puerto efímero que el test POSEE** (no `localhost:<dominio>` — el eje que C89 cerró) → slot `security` healthy+version, y **el store NO cambia** (el pull es read-only: **healthy ⇏ registrado**, la independencia en una dirección); (2) **PUSH** → el SDK real `register()+heartbeat()` contra el MISMO gateway → +1 `service.registered` y +1 `service.heartbeat` con source=cybertools; (3) **coexistencia** → el gateway ve al dominio vivo por AMBOS canales, simultáneos y de acuerdo. Lo que C83 vio a mano, ahora e2e.
  - **Test 2 (la otra cara de la independencia):** PUSH **sin** PULL deja el slot del registry **sin tocar** — `register()` es un evento, no un alta; *"los dominios no pueden darse de alta, Micelia los descubre"* (C83), ahora ejecutable.
- **Verify:** **verde** — `make verify` sin postgres: `1610 passed, 12 skipped` (+2 e2e que skip sin pg), cobertura ~95%.
- **Ejercido con postgres real:** contenedor efímero aislado `:5433` (propio, sin tocar el `idm-postgres` del usuario ni `.env`) → **2 passed**. **Mutation-verified:** `append_event` no-op rompe la coexistencia PUSH (+1); `pull healthy→False` rompe el PULL. Fuente restaurada (git diff limpio).
- **Higiene:** e2e in-process (ASGITransport + servidor loopback propio), nada de gateway/uvicorn arrancado. Postgres efímero **eliminado**, **podman machine detenida**. `:8888/:5433` libres. `.env` no leído. Sin residuales.

**HITO — contrato SDK↔registry COMPLETO, coexistente y verificado independiente:** las 3 piezas que C83 vio en vivo, pineadas por separado (register C86, heartbeat C88, pull C90) y ahora demostradas **coexistiendo sobre el mismo dominio en el mismo gateway, y desacopladas** (C92). Los dos mecanismos independientes de C83, juntos y verificados como tales.

**DECISIÓN PENDIENTE (para Jessicache):** **ninguna nueva.** Siguen abiertas **DP-17** (biohack no arrancable en 3.14, C91), **DP-16** (venvs huérfanos, C87 — emparejada con DP-17: pinning de python por proyecto), la **menor de C84** (slot `devtools`) y **DP-1..DP-4** (INFRA del funnel).

**Mañana (Ciclo 93):** con el contrato SDK↔registry **completo y coexistente**, se acaban los tramos internos obvios que quedaban. Los vectores vivos, por valor: **(a)** si Jessicache decide **DP-17(b)** (python 3.13 vía pyenv para biohack) o **DP-16**, desbloquear el 3er dominio live — el contrato de su endpoint ya está pineado (C91), faltaría la verificación viva. **(b)** El vector que NUNCA se ha tocado en 92 ciclos y es el objetivo final del proyecto: empezar a **preparar** (no decidir) **DP-1..DP-4** — el funnel `register`/`login`/`micelia` en idmmortality.com; lo autonomizable es *docs/scaffolding/checklist de infra*, no las decisiones de DNS/TLS/hosting (esas son de Jessicache). Es la deuda estratégica más grande: 92 ciclos endureciendo el núcleo, cero avance hacia el destino del funnel. **(c)** Un dominio de mentira completo (register+heartbeat+pull en loop) como smoke de integración continua. Recordatorio honesto C66–C92: **pinear los DOS lados de un contrato Y su independencia** (hoy: coexisten pero no se acoplan); **la hermeticidad es POSEER el recurso, no evitar la red** (loopback propio); **ejercer el sistema real rinde**; **el marker/nombre/valor efectivo no es el contrato**; **recrear entorno gitignored = OK, tocar código/requirements del hermano = decisión del dueño**. Local-first M1: jamás `docker-full`; parar/eliminar todo lo que se arranque; `.env` intocable.

**Estado: IMPLEMENTADO ✅**

---

## 2026-07-19 — Ciclo 91 (**el arranque live de biohack (3er dominio) queda BLOQUEADO por un blocker más duro que el de canela: sus deps están CLAVADAS a versiones de 2023 sin wheel para python 3.14. No fuerzo (tocar sus pins es código del hermano) — extraigo el valor verificable SIN arrancar: pineo el contrato cross-repo de su endpoint de health, que era el ÚNICO distinto y no lo verificaba nada**): tarea (a) del plan de C90. Objetivo era biohack live tras cybertools (C83) y canela (C87).

**El blocker, probado (no supuesto):** biohack no tiene `.venv` (ausente, no huérfano). Al crearlo en python **3.14** (única versión del sistema; brew desinstaló `python@3.13`, C87) e intentar `pip install`, su `requirements.txt` **clava** `numpy==1.25.2` (+ `scipy==1.11.4`, `scikit-learn==1.3.2`, `pandas==2.1.4`, todas de 2023). Probado con `pip install --only-binary=:all: numpy==1.25.2` → **`No matching distribution`**: los wheels `cp314` de numpy empiezan en **2.3.2**; 1.25.2 ni existe para 3.14. **Diferencia clave con canela (C87):** canela usaba `>=` → recrear el venv en 3.14 trajo wheels nuevos y arrancó; biohack **PINEA versiones viejas** → recrear no basta, habría que **bumpear sus requirements**, que es tocar código del hermano (**fuera de scope**; además biohack tiene reglas propias estrictas de no-romper-nada en su CLAUDE.md).

- **Hecho:** 1 commit — `6021085` `test(biohack): pin cross-repo del endpoint de health (arranque live BLOQUEADO por deps)`. Valor extraído **sin arrancar**: biohack es el `health` slot y el **ÚNICO** dominio cuyo endpoint de salud NO es `/health` — el registry lo sondea en **`/api/v1/service-health`** (`HEALTH_ENDPOINTS["health"]`). Ese contrato de **dos lados** no lo verificaba nada junto: si biohack renombra el endpoint o Micelia cambia `HEALTH_ENDPOINTS`, el registry sondea un **404** y el slot `health` queda unhealthy **para siempre en silencio** (modo de fallo de DP-14). `tests/test_biohack_health_contract_codex.py` (4 tests, espejo del pin de canela DP-8): (1) Micelia sondea el slot en `/api/v1/service-health` (símbolo real); (2) `ServiceStatus` acepta el `version="0.1.0"` que biohack emite — **contraste con canela**, que lo omite (DP-8); (3) biohack **SIRVE** exactamente esa ruta (read-only sobre su `main.py`; SKIP si ausente); (4) su `service-health` emite `version+service+status` a nivel superior (parser **consciente del dict anidado `dependencies`**, verificado que no filtra claves anidadas).
- **Verify:** **verde** — `make verify`: `1610 passed, 10 skipped` (+4), cobertura ~95%.
- **Disciplina C60–C91:** el pin del lado Micelia está **mutation-verified** (cambiar `HEALTH_ENDPOINTS["health"]` lo rompe); el parser se verificó **no-vacuo** (extrae `status/version/service/capabilities/dependencies` y NO las anidadas `database/redis/ml_onnx`). Read-only sobre biohack.
- **Higiene:** el `.venv` parcial que creé para probar el blocker se **eliminó** — biohack queda **como estaba (sin venv)**. Nada arrancado, cero infra, `.env` no leído. Sin residuales.

**DECISIÓN PENDIENTE (para Jessicache):** **NUEVA (DP-17): biohack no es arrancable en local con el toolchain actual.** Sus deps clavadas (numpy 1.25.2 etc.) no tienen wheel para python 3.14 y el sistema ya no tiene 3.13. Opciones: **(a)** bumpear `requirements.txt` de biohack a versiones con wheel `cp314` (numpy≥2.3, scipy/scikit/pandas al día) y validar que su código sobrevive el salto de API (numpy 2.x / pandas 2.2 rompieron cosas) — **deep-work en el hermano, decisión del dueño**; **(b)** instalar un python 3.13 (pyenv) dedicado para el venv de biohack sin tocar su código — resuelve YA el arranque, es la vía menos invasiva, pero añade un toolchain; **(c)** dejarlo: biohack se integra por contrato (pineado hoy) y se arranca cuando (a)/(b) se resuelva. **Emparejada con DP-16** (venvs huérfanos por bumps de brew): ambas son el mismo problema de **pinning de python por proyecto**. Siguen abiertas la **menor de C84** (slot `devtools`) y **DP-1..DP-4** (INFRA del funnel).

**Mañana (Ciclo 92):** **(a)** El e2e cross-mecanismo que sigue solo en prosa (anotado en C90): un dominio VIVO que hace PUSH (register/heartbeat) mientras el registry hace PULL sobre él — hoy cada mitad se pinea por separado; con **cybertools o canela** (los 2 que SÍ arrancan) se puede demostrar en vivo que ambos mecanismos coexisten sobre el mismo dominio. **(b)** Si Jessicache decide DP-17(b), volver a biohack con un python 3.13 y completar su arranque live (el contrato ya está pineado; faltaría la verificación viva del 404-si-drift). **(c)** Empezar a **preparar** DP-1..DP-4 (docs/scaffolding del funnel, sin decidir infra). Recordatorio honesto C66–C91: **un blocker real se documenta y se rodea, no se fuerza** (hoy: no bumpeé deps del hermano; extraje el contrato leíble); **ejercer rinde el blocker que la lectura no anticipa** (el `pip install` reveló el pin incompatible que un vistazo al requirements no gritaba); **pinear el contrato cross-repo cuando el arranque no se puede** (el endpoint distinto de biohack era un DP-14 latente sin guard); **recrear/crear entorno gitignored = OK; bumpear requirements del hermano = decisión del dueño**. Local-first M1: jamás `docker-full`; parar/eliminar todo lo que se arranque; `.env` intocable.

**Estado: IMPLEMENTADO ✅** (arranque live BLOQUEADO y documentado como DP-17; el ciclo entrega el pin de contrato cross-repo, verify verde — no queda en "solo plan")

---

## 2026-07-19 — Ciclo 90 (**cierro el ÚLTIMO tramo sin pin del contrato SDK↔registry: el PULL. C86 pineó el register, C88 el heartbeat (ambos PUSH); hoy el PULL — el sondeo que el registry hace por su cuenta, visto a mano en C83/C87 pero nunca ejecutable en 90 ciclos. Con esto el contrato queda completo**): tarea (a) del plan de C89. C83 descubrió que Micelia tiene DOS mecanismos INDEPENDIENTES y que hay que verificarlos por separado: **PUSH** (el dominio hace `POST /api/v1/events` — register + heartbeat → Event Store) y **PULL** (el registry SONDEA `{service.url}{endpoint}` por su cuenta: 200 → `healthy` + `version` + latencia; conexión rechazada → `unhealthy`/`connection_refused`). El PUSH quedó pineado en C86/C88; el PULL solo se había visto A MANO (C83 `security`→healthy; C87 `research`→healthy con `version=None` por DP-8).

- **Hecho:** 1 commit — `c789f8a` `test(registry): guard ejecutable del PULL — el último tramo del contrato SDK↔registry`. `tests/test_registry_pull_codex.py` (5 tests) ejerce el sondeo **DE VERDAD**: un servidor HTTP real en un **puerto efímero que el test POSEE** (`127.0.0.1:0` → lo asigna el SO), sondeado por el `ServiceRegistry` real con un `httpx.AsyncClient` real → el round-trip HTTP entero (status, parseo de `version`, latencia), lo que un cliente mockeado no puede dar. Casos: (1) 200 con version → healthy + version + latencia medida (lo de cybertools en C83); (2) 200 **SIN** version → healthy, `version=None` (la forma de canela, DP-8 confirmada en C87 — ahora atada a un pin ejecutable); (3) non-200 (503) → unhealthy; (4) puerto muerto → unhealthy, `error='connection_refused'`; (5) **recovery**: caído → vivo re-sondeando (el registry RE-evalúa estado, no lo cachea — el 'recovery' que C83 vio en vivo).
- **Verify:** **verde** — `make verify`: `1606 passed, 10 skipped` (+5), cobertura ~95%. Estable en **3 corridas** (el servidor loopback no introduce flakiness).
- **Hermeticidad by-design (aplicando la lección C85→C89 que acabo de barrer):** el guard **NO** sondea `localhost:<puerto de dominio>` — eso reintroduciría el eje que C89 cerró. El test **levanta y controla** su propio servidor en un puerto que pide al SO (bind `:0`); no asume nada del entorno. Sin infra, sin postgres, **in-process** — cero servicios/contenedores arrancados.
- **Disciplina C60–C90:** los pins están **mutation-verified** — `healthy` siempre True rompe el test non-200; no extraer `version` rompe el de version; string de error distinto rompe el de connection_refused. Fuente restaurada (0 residuo de mutante).
- **Higiene:** cambio test-only in-process; nada arrancado, nada que parar; `.env` no leído.

**HITO — contrato SDK↔registry COMPLETO y ejecutable:** las 3 piezas que C83 vio en vivo pero que 82 ciclos nunca pinearon están ahora pineadas de punta a punta — **register** (C86, `service.registered`→store), **heartbeat** (C88, `service.heartbeat`→store + loop) y **PULL** (C90, sondeo→healthy/version/latencia). Los dos mecanismos independientes que C83 identificó, ambos cubiertos.

**DECISIÓN PENDIENTE (para Jessicache):** **ninguna nueva.** Siguen abiertas **DP-16** (venvs huérfanos, C87), la **menor de C84** (slot `devtools`) y **DP-1..DP-4** (INFRA del funnel).

**Mañana (Ciclo 91):** con el contrato SDK↔registry cerrado, los vectores vivos por valor: **(a)** verificar **biohack** en vivo (3er dominio; su venv casi seguro huérfano de python3.13 → el launcher de C87 lo dirá; recrear en 3.14 como canela en C87) — sería el 3er dominio real tras cybertools/canela, y de paso ejercita que el health-endpoint de biohack (`/api/v1/service-health`, distinto de los demás) se sondea bien. **(b)** El eslabón que aún vive solo en prosa: un **e2e cross-mecanismo** — un dominio que hace PUSH (register) Y al mismo tiempo el registry hace PULL sobre él, demostrando que ambos coexisten sobre el mismo dominio vivo (hoy cada mitad se pinea por separado). **(c)** Si Jessicache prioriza: empezar a **preparar** DP-1..DP-4 (docs/scaffolding del funnel `register`/`login`/`micelia` en idmmortality.com, sin tomar decisiones de infra). Recordatorio honesto C66–C90: **ejercer el sistema real rinde lo que el mock no da** (hoy: el round-trip HTTP real, con su parseo de version y su latencia); **la hermeticidad es POSEER el recurso, no evitar la red** (el test levanta su propio server en un puerto propio — real pero controlado); **pinear los dos lados de un contrato por separado** (PUSH y PULL son mecanismos independientes, C83); **el marker/nombre/valor efectivo no es el contrato**; **recrear entorno gitignored = OK, tocar código del hermano = NO**. Local-first M1: jamás `docker-full`; parar todo lo que se arranque; `.env` intocable.

**Estado: IMPLEMENTADO ✅**

---

## 2026-07-19 — Ciclo 89 (**hago EL BARRIDO de no-hermeticidad que aplacé 3 ciclos (C86→C87→C88 lo prometieron y no lo hicieron) — "ejercer, no declarar". Encuentro un 3er eje que las auditorías anteriores no vieron: tests que pinean un DEFAULT de config leyéndolo a través de `Settings()`, que layerea el `.env` real encima**): tarea (a) del plan de C88, la de más deuda acumulada. La lección de C85→C86 fue que *"barrí la clase entera"* hay que **ejercerlo**; llevaba 3 ciclos anotado sin hacer. Hoy se hace, sistemático.

**Método:** grep dirigido por vector + lectura de cada candidato (no confiar el grep). Ejes de "el resultado del test depende del ENTORNO de la máquina, no de lo que el test fija":

1. **Infra ausente** (`app.state.*_store is None`) → ya cubierto por `no_infra` (C86). ✓
2. **Sondeo de puertos de dominio** (lifespan real) → ya cubierto por `no_domain_probes` (C85/C86); confirmado que solo 3 ficheros bootean el lifespan y los 3 usan las fixtures. ✓
3. **NUEVO — default de config vía `Settings()` env-layered:** `test_registry_domains_codex.py` pinea —lo dicen sus docstrings ("por defecto")— el DEFAULT de `config.py`, pero lo leía instanciando `Settings()`, que **layerea el `.env` real** (`env_file=".env"`; **Micelia TIENE un `.env` de 4.6 KB**) y las env-vars del proceso encima del default. → pinea el valor EFECTIVO de la máquina, no el default que dice pinear.

- **Hallazgo, probado empíricamente (no argumentado):** `OLLAMA_CODE_ENABLED=true` → `Settings().ollama_code_enabled == True` → `test_devtools_phantom_is_disabled_by_default` **FALLABA** aunque el default de config.py siga en False; idéntico con `IMPERIO_LAB_ENABLED=false` sobre el pin de `imperio_lab`. Tres asserts afectados (ollama_code_enabled, imperio_lab_enabled, imperio_lab_url).
- **Hecho:** 1 commit — `fd98eb9` `test(registry-domains): pinear el DEFAULT de config, no el valor env-layered (hermeticidad)`. Helper `_config_default(field)` = `Settings.model_fields[field].default` → lee el literal declarado en config.py **sin construir Settings** → cero capas de entorno. Es la forma plenamente hermética del patrón `Settings(_env_file=None, …)` que **`test_core_config_codex.py` ya usaba** (el codebase conocía el peligro; este fichero se lo había saltado). El test que reactiva vía kwarg (`Settings(ollama_code_enabled=True)`) se deja **intacto**: el kwarg tiene prioridad máxima → ya era hermético.
- **Verify:** **verde** — `make verify`: `1601 passed, 10 skipped` (sin cambio de conteo: arreglo in-place de 3 asserts, no tests nuevos), cobertura ~95%.
- **Hermeticidad demostrada (before/after):** los 5 tests del fichero pasan; y con `OLLAMA_CODE_ENABLED=true IMPERIO_LAB_ENABLED=false` la **SUITE COMPLETA pasa** (1581 passed, 0 fallos) — exactamente las env-vars que **antes la rompían**. El arreglo es load-bearing y verificado ejerciendo, no declarando.
- **El resto del suite ES hermético (barrido completo, cada candidato leído):** registry-health y event-bus **mockean su cliente** (`AsyncMock(spec=httpx.AsyncClient)` / `redis.from_url` parcheado); `test_idm_sdk` **mockea `_http_client`** (el `micelia_url=:8888` es solo string, sin conexión real); `test_entire_session` **parchea `shutil.which`** en todos los tests (nunca depende del PATH real del binario `entire`); `test_mcp_generator` usa **`chdir(tmp_path)` + `subprocess.Popen` mockeado**; `test_provider_store` usa `tmp_path`; e2e usa **respx + `random.Random` sembrado**; `test_policy_engine` usa **fechas relativas** (`yesterday`, `(month%12)+1` — robustas en cualquier fecha, incl. día 1); los `assert body["services"][x] == settings.x_enabled` de `main_boot` son **auto-consistentes** (comparan la app contra el MISMO `settings`, no un default fijo) y encima viven bajo `no_infra`.
- **Higiene:** cambio test-only, cero servicios arrancados, cero infra, `.env` **no leído** (el hallazgo se probó con env-vars del proceso y con `model_fields`, sin abrir el fichero). Nada residual.

**DECISIÓN PENDIENTE (para Jessicache):** **ninguna nueva.** Los 3 ejes de no-hermeticidad conocidos (infra, sondeo de dominio, default-vía-Settings) están **cerrados**; el barrido no dejó más candidatos. Siguen abiertas **DP-16** (venvs huérfanos, C87), la **menor de C84** (slot `devtools`) y **DP-1..DP-4** (INFRA del funnel).

**Mañana (Ciclo 90):** con el barrido saldado, los vectores vivos por valor: **(a)** el eslabón que sigue sin pin ejecutable — el **registry PULL** virando un dominio a `healthy` (visto a mano en C83/C87, nunca pineado): test con un dominio de mentira sirviendo `/health` en un **puerto real controlado** (bind efímero, NO `localhost:<dominio>` — eso reintroduciría el eje ya cerrado) + el registry sondeándolo → cerraría el ÚLTIMO tramo del contrato SDK↔registry. **(b)** Verificar **biohack** (3er dominio live; su venv casi seguro huérfano de python3.13 → el launcher de C87 lo dirá; recrear en 3.14 como canela). **(c)** Si Jessicache prioriza: empezar a **preparar** DP-1..DP-4 (docs/scaffolding del funnel, sin tomar decisiones de infra). Recordatorio honesto C66–C89: **la deuda de "lo barro mañana" se paga o se acumula — hoy se pagó tras 3 ciclos**; **ejercer > declarar** (el 3er eje salió probando env-vars, no leyendo); **un `Settings()` en un test lee el `.env` real — para pinear un DEFAULT usa `model_fields[...].default`**; **el marker/nombre/valor efectivo no es el contrato**; **recrear entorno gitignored = OK, tocar código del hermano = NO**. Local-first M1: jamás `docker-full`; parar todo lo que se arranque; `.env` intocable.

**Estado: IMPLEMENTADO ✅**

---

## 2026-07-19 — Ciclo 88 (**cierro la 2ª mitad del contrato de C83 que C86 dejó pendiente: el HEARTBEAT. C86 pineó `register()`; hoy pineo el latido — el evento que C83 vio 2 veces en el store en vivo pero que 88 ciclos nunca habían dejado ejecutable**): tarea (a) del plan de C87. C83 observó en el store, arrancando cybertools de verdad: `{'service.registered': 1, 'service.heartbeat': 2, 'system.initialized': 1}`. C86 dejó pineado el `register`; el `service.heartbeat` (y su loop de 30s) seguía verificado solo A MANO. C88 lo cierra.

**El contrato del latido (leído, no asumido):** `heartbeat()` del SDK recorre el MISMO camino que `register()` — `POST /api/v1/events` → `verify_auth` → Event Store — con `event_type='service.heartbeat'`, `action='update'`, `tags=['heartbeat']` y un `payload` que es el `health_response()` del SDK (`status/version/service/category/port/capabilities/uptime_seconds/dependencies`, vía `models.HealthResponse.to_dict()`). Encima hay un `_heartbeat_loop` que lo repite cada `heartbeat_interval` s.

- **Hecho:** 1 commit — `2c30f41` `test(heartbeat): extiende el guard de C83 a la 2ª mitad — service.heartbeat → store`. 3 tests nuevos en el guard existente (`tests/test_register_store_contract_codex.py`): (1) **gate store** — latido sin Event Store → 503 → `heartbeat()` devuelve **False en silencio** (la misma degradación silenciosa que C83 documentó: un dominio "vivo" cuyos latidos se pierden si el store cae); (2) **el LOOP** — `_heartbeat_loop` llama a `heartbeat()` una vez por intervalo, pineado **determinista sin reloj de pared** (se parchea `asyncio.sleep` en el módulo del SDK y se cuentan los latidos — mismo patrón que `test_continuous_monitoring` del registry); (3) **cadena completa** — gateway REAL (lifespan) + SDK REAL → `heartbeat()` deja un `service.heartbeat` con `source='cybertools'` y el payload de `health_response` íntegro (service/status/version/port) en el store. Los 2 primeros corren siempre (sin pg); el 3º `skipif` sin postgres, reusando `require_postgres`+`no_domain_probes`+`TEST_API_KEY` de conftest (la infra de test que C86 dejó montada).
- **Verify:** **verde** — `make verify` sin postgres: `1601 passed, 10 skipped` (+2 tests que corren siempre, +1 e2e que skip), cobertura ~95%.
- **Ejercido con postgres real (no solo skip):** postgres efímero aislado en `:5433` (contenedor propio, creds propias — sin tocar el `idm-postgres` del usuario ni `.env`) → **los 7 tests del fichero pasan** (register×4 de C86 + heartbeat×3 de hoy): las DOS entradas que C83 vio en el store (`service.registered` + `service.heartbeat`) quedan ambas pineadas de punta a punta.
- **Disciplina C60–C88:** los 2 pins nuevos con lógica propia están **mutation-verified** — sin `await self.heartbeat()` en el loop, el test del loop **falla** (0≠3 latidos); con `event_type` equivocado en el SDK, el e2e **falla** en el delta del store. SDK restaurado y verificado (0 mutantes). El 3er test (gate 503) reusa el mismo camino ya mutation-verified de register en C86.
- **Higiene:** cero residuales — los tests corren in-process (ASGITransport), no arranqué gateway ni uvicorn. Postgres efímero **eliminado**, **podman machine detenida** (la arranqué solo para esto). `:8888/:5433` libres. Repos hermanos no tocados; el único código mutado (`app/sdk/client.py`) es de Micelia y quedó restaurado; `.env` no leído.

**DECISIÓN PENDIENTE (para Jessicache):** **ninguna nueva.** El contrato de C83 (register + heartbeat) queda **completo y ejecutable**. Siguen abiertas: **DP-16** (venvs huérfanos por bumps de python de brew — C87, menor, de proceso), la **menor de C84** (slot `devtools` desactivado) y **DP-1..DP-4** (INFRA del funnel).

**Mañana (Ciclo 89):** **(a)** El barrido que C86 y C87 dejaron anotado 2 ciclos seguidos y aún sin hacer — **el más honesto por deuda acumulada**: buscar OTROS tests que codifiquen una suposición de entorno no fijada (grep de `is None` sobre `app.state.*_store`, de puertos de infra/dominio en asserts sin fixture como `no_infra`/`no_domain_probes`). La lección C85→C86 fue que "barrí la clase entera" hay que **ejercerlo**; van 2 ciclos prometiéndolo. **(b)** Con canela ya arrancable (C87) y el contrato SDK completo (C88), el eslabón que sigue sin pin ejecutable es el **registry PULL** virando un dominio a `healthy` — hoy y en C83/C87 se vio a mano (research/security→healthy); un test con un dominio de mentira sirviendo `/health` en un puerto efímero + el registry sondeándolo lo cerraría (ojo: cae en la clase no-hermética → usar puerto real controlado, no `localhost:<dominio>`). **(c)** Verificar **biohack** (3er dominio; su venv casi seguro huérfano de python3.13 → el launcher de C87 ya lo dirá). Recordatorio honesto C66–C88: **la deuda de "lo barro mañana" se acumula — el barrido de entorno lleva 2 ciclos aplazado, priorizarlo**; **ejercer el sistema real rinde lo que la lectura no da**; **pinear determinista > con reloj de pared** (hoy: el loop se pineó parcheando sleep, no durmiendo); **el marker/nombre no es el contrato**; **recrear entorno gitignored = OK, tocar código del hermano = NO**. Local-first M1: jamás `docker-full`; parar/eliminar todo lo que se arranque; `.env` intocable.
